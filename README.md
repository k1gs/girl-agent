...
## Web UI на сервере (VPS)

Для развертывания girl-agent с веб-интерфейсом на вашем сервере (с Docker и Nginx), склонируйте репозиторий и используйте скрипт:
...
Скрипт:
1. Поможет получить бесплатный SSL сертификат (Let's Encrypt), если у вас есть домен. Либо сгенерирует самоподписанный (если у вас только IP).
2. Создаст шаблон настроек bot.json.
3. Поднимет бота и Nginx.
4. Выдаст безопасную ссылку с уникальным токеном для доступа к дашборду.
